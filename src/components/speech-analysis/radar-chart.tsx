...
import type { SpeechDimension } from "@/components/speech-analysis/types";

const AXES: SpeechDimension[] = ["confidence", "diplomaticTone", "evidenceDensity", "structure", "pacing"];
const SIZE = 300;
const CENTER = SIZE / 2;
const MAX_R = 100;
...
export function RadarChart({ values }: { values: Record<SpeechDimension, number> }) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Speech diagnostic radar chart"
    >
      {RINGS.map((f) => (
        <polygon
          key={f}
