import type { PositionPaperCriterion } from "@/components/position-paper/types";

const AXES: PositionPaperCriterion[] = ["policyAlignment", "topicAnalysis", "proposedSolutions", "formatting"];

// Short forms for the chart's radial labels — the full CRITERION_LABELS run
// too wide at the left/right cardinal points of a 4-axis radar and clip.
const AXIS_LABELS: Record<PositionPaperCriterion, string> = {
...
export function RadarChart({ values }: { values: Record<PositionPaperCriterion, number> }) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Position paper score radar chart"
    >
      {RINGS.map((f) => (
        <polygon
          key={f}
