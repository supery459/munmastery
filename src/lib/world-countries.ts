/**
 * Every UN member state (193) plus the two non-member observer states (the
 * Holy See and Palestine) — 195 entries total. This is the searchable
 * delegation list used by Conference Portfolios, the Debate Simulator, and
 * the Position Paper Grader. It's deliberately separate from simulator/data's
 * `COUNTRIES`: that list carries hand-written diplomatic "voice" (openers,
 * stance verbs, priorities) for a curated subset of delegations, while this
 * one is just code+name so every real country can be searched and selected.
 */
